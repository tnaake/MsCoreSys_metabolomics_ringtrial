...
qM_msLevel1[["lab"]] <- ifelse(
    ...
    yes = "lab_Schmidln", no = qM_msLevel1[["lab"]])

## qM_msLevel2
qM_msLevel2 <- qM_msLevel2 |>
    as.data.frame()
qM_msLevel2[["lab"]] <- NA
qM_msLevel2[["lab"]] <- ifelse(
    test = grepl(rownames(qM_msLevel2), pattern = "lab_Bindila"), 
    yes = "lab_Bindila", no = qM_msLevel2[["lab"]])
qM_msLevel2[["lab"]] <- ifelse(
    test = grepl(rownames(qM_msLevel2), pattern = "lab_Hopf"), 
    yes = "lab_Hopf", no = qM_msLevel2[["lab"]])
qM_msLevel2[["lab"]] <- ifelse(
    test = grepl(rownames(qM_msLevel2), pattern = "lab_Poschet"), 
    yes = "lab_Poschet", no = qM_msLevel2[["lab"]])
qM_msLevel2[["lab"]] <- ifelse(
    test = grepl(rownames(qM_msLevel2), pattern = "lab_Schmidlin"), 
    yes = "lab_Schmidln", no = qM_msLevel2[["lab"]])
```

Add information on ionization mode in the `ionization_column`.

```{r}
rownames(qM_msLevel1)
...
plotMetric(qM_msLevel1, metric = "areaUnderTic", plotly = FALSE) +
    facet_grid(~ rep(qM_msLevel1$lab, 5), scales = "free_x") + 
